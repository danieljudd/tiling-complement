...
# screensaver off because it runs during fullscreen video
exec xset s off &
...
while true; do
	LOCALTIME=$(date +'%A, %d-%m-%Y %H:%M')
        ...
        # Enable if you have a battery (laptop) and add "BAT: $BAT%" to xsetroot list
	# BAT=$(cat '/sys/class/power_supply/BAT0/capacity')
        ...
        xsetroot -name " $LOCALTIME | MEM: $MEM | CPU: $CPU | $WEATHER | VOL: $VOL"
        sleep 10s
done &
